...
@torch.no_grad()
def evaluate(model, val_loader, device, global_step=0, aim_run=None, amp=False):
    model.eval()
    total_correct = 0
    total_pixels = 0
    ...
    for puzzle, solution in val_loader:
        ...
        with torch.amp.autocast("cuda", enabled=amp):
            pred = model(puzzle)
            val_loss += F.mse_loss(pred, solution).item()
        val_batches += 1

        gt_bin = ((solution + 1) / 2 > 0.5).float()
        pred_bin = ((pred + 1) / 2 > 0.5).float()

        total_correct += (pred_bin == gt_bin).sum().item()
        total_pixels += gt_bin.numel()
        total_intersection += (pred_bin * gt_bin).sum().item()
        total_union += ((pred_bin + gt_bin) > 0).sum().item()
...
def main():
    parser = argparse.ArgumentParser(description="Direct maze solver training")
    ...
    parser.add_argument("--aim_experiment", type=str, default="DiffuMaze-Direct")
    ...
    model_arch_defaults = {
        "unet":        {"base_ch": 64, "ch_mults": "1,2,4,8", "num_res_blocks": 2},
        "unet_small":  {"base_ch": 64, "ch_mults": "1,2,4",   "num_res_blocks": 2},
        "unet_xsmall": {"base_ch": 32, "ch_mults": "1,2,4",   "num_res_blocks": 1},
        "unet_xxsmall": {"base_ch": 16, "ch_mults": "1,2,4",   "num_res_blocks": 1},
    }
    ...
    in_channels = puzzle_sample.shape[0]
    out_channels = solution_sample.shape[0]
    ch_mults = parse_ch_mults(args.ch_mults)

    model_class = {"unet_small": DirectUNetSmall, "unet_xsmall": DirectUNetXSmall, "unet_xxsmall": DirectUNetXXSmall}.get(args.model, DirectUNet)
    model = model_class(
